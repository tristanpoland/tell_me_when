...
impl PlatformWatcher {
    ...
    async fn start_monitoring_task(&self, watch_handle: &WindowsWatchHandle, config: FsWatchConfig) {
        // Copy the handles as raw values for Send safety
        ...
        std::thread::spawn(move || {
            ...
            loop {
                unsafe {
                    ...
                    // Wait for the event to be signaled - this is a blocking OS call
                    let wait_result = WaitForSingleObject(event_handle, INFINITE);
                    ...
                    if bytes_returned > 0 {
                        Self::process_notifications(&buffer[..bytes_returned as usize], &watched_path, &event_sender, &handler_id);
                    }
                    
                    // Restart the monitoring for the next batch of events
                    let notify_filter = FILE_NOTIFY_CHANGE_FILE_NAME
                        | FILE_NOTIFY_CHANGE_DIR_NAME
                        | FILE_NOTIFY_CHANGE_LAST_WRITE
                        | FILE_NOTIFY_CHANGE_CREATION
                    ...
                    let mut overlapped_restart = std::mem::zeroed::<OVERLAPPED>();
                    overlapped_restart.hEvent = event_handle;
                    let mut bytes_returned_restart = 0u32;

                    let restart_success = ReadDirectoryChangesW(
                        directory_handle,
                        buffer.as_mut_ptr() as *mut _,
                        buffer.len() as u32,
                        if config.watch_subdirectories { 1 } else { 0 },
                        notify_filter,
                        &mut bytes_returned_restart,
                        &mut overlapped_restart,
                        None,
                    );

                    if restart_success == 0 {
                        let error = GetLastError();
                        if error != ERROR_IO_PENDING {
                            log::error!("Failed to restart ReadDirectoryChangesW: {}", error);
                            break;
                        }
                    }
                }
