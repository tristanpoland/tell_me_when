...
pub async fn start_process_monitoring(
    ...
    // Use proc filesystem monitoring for Linux
    task::spawn_blocking(move || {
        monitor_via_proc_fs(&config, &system, &previous_processes, &is_running, sender, handler_id);
    }).await.map_err(|e| TellMeWhenError::RuntimeError(format!("Process monitoring task failed: {}", e)))?;

    Ok(())
}

fn monitor_via_proc_fs(
    config: &ProcessConfig,
    system: &Arc<Mutex<System>>,
    previous_processes: &Arc<Mutex<HashMap<u32, ProcessSnapshot>>>,
    is_running: &Arc<Mutex<bool>>,
    ...
    log::info!("Starting Unix process monitoring via /proc filesystem");
